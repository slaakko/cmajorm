...
void ProcessMessageQueues::CloseAll(cmsx::machine::Process* process)
{
    for (auto md : mds)
    {
        if (IsMsgQOpen(md))
...
int32_t MsgQ(cmsx::kernel::Process* process, int64_t nameAddr)
{
    if (nameAddr == 0)
    {
...
void CloseMsgQ(cmsx::machine::Process* process, int32_t md)
{
    CloseMsgQ(process, md, true);
}

void CloseMsgQ(cmsx::machine::Process* process, int32_t md, bool remove)
{
...
void WaitMsgLocked(std::unique_lock<std::recursive_mutex>& lock, cmsx::machine::Process* process, int32_t md)
{
    MsgQueue* queue = MsgQueues::Instance().Get(md);
...
void WaitMsg(cmsx::machine::Process* process, int32_t md)
{
    cmsx::machine::Machine* machine = process->GetMachine();
#if (LOCK_DEBUG)
...
int32_t GetMsgQueueLength(cmsx::machine::Process* process, int32_t md)
{
    MsgQueue* queue = MsgQueues::Instance().Get(md);
    return queue->Length();
}

int32_t GetMsgSize(cmsx::machine::Process* process, int32_t md)
{
    MsgQueue* queue = MsgQueues::Instance().Get(md);
    if (queue->IsEmpty())
    {
...
void GetMsg(cmsx::kernel::Process* process, int32_t md, int64_t bufferAddr)
{
    if (bufferAddr == 0)
    {
