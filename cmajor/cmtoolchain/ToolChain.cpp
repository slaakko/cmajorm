...
Tool::Tool() : name(), commandName(), outputFileExtension(), debugInformationFileExtension(), assemblyFileExtension(), args()
{
}
...
std::unique_ptr<soulng::util::JsonValue> Tool::ToJson() const
{
    ...
    std::unique_ptr<soulng::util::JsonValue> fieldValue5 = sngjson::json::ToJson(args);
    object->AddField(U"args", std::move(fieldValue5));
    return object;
}
