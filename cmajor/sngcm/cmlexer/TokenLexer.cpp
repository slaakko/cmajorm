...
#include <sngcm/cmlexer/TokenLexerClassMap.hpp>

using namespace soulng::lexer;
using namespace TokenLexerTokens;

TokenLexer::TokenLexer(const std::u32string& content_, const std::string& fileName_, int fileIndex_) : soulng::lexer::Lexer(content_, fileName_, fileIndex_),
    inBlockComment()
{
    SetKeywordMap(TokenLexerKeywords::GetKeywordMap());
}

TokenLexer::TokenLexer(const char32_t* start_, const char32_t* end_, const std::string& fileName_, int fileIndex_) : soulng::lexer::Lexer(start_, end_, fileName_, fileIndex_),
    inBlockComment()
{
    SetKeywordMap(TokenLexerKeywords::GetKeywordMap());
}

int TokenLexer::NextState(int state, char32_t c)
{
    int i = TokenLexerClassMap::GetClass(c);
    switch (state)
    {
        case 0:
        {
