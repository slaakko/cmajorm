...
#include <sngjson/json/JsonSchemaClassMap.hpp>

using namespace soulng::lexer;
using namespace JsonSchemaTokens;

JsonSchemaLexer::JsonSchemaLexer(const std::u32string& content_, const std::string& fileName_, int fileIndex_) : soulng::lexer::Lexer(content_, fileName_, fileIndex_),
    parsingIncludeDeclaration()
{
    SetKeywordMap(JsonSchemaKeywords::GetKeywordMap());
}

JsonSchemaLexer::JsonSchemaLexer(const char32_t* start_, const char32_t* end_, const std::string& fileName_, int fileIndex_) : soulng::lexer::Lexer(start_, end_, fileName_, fileIndex_),
    parsingIncludeDeclaration()
{
    SetKeywordMap(JsonSchemaKeywords::GetKeywordMap());
}

int JsonSchemaLexer::NextState(int state, char32_t c)
{
    int i = JsonSchemaClassMap::GetClass(c);
    switch (state)
    {
        case 0:
        {
...
int JsonSchemaLexer::GetTokenId(int statementIndex)
{
