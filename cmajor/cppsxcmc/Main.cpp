...
{
#ifdef _WIN32
    std::cout << "Cmajor System X compiler version with C++ backend" << version << " for Windows x64" << std::endl;
#else
    ...
    std::cout << "Options:\n" <<
        ...
        "--fnxml (-fx)\n" <<
        "   write function XML files to lib directory\n" <<
        "--dtxml (-dx)\n" <<
        "   write dominator tree XML files to lib directory\n" <<
        "--pass=PASS (-p=PASS)\n" <<
        ...
        else
        {
            ...
            for (int i = 1; i < argc; ++i)
            {
                ...
                if (!arg.empty() && arg[0] == '-')
                {
                    ...
                    else if (arg == "--fnxml" || arg == "-fx")
                    {
                        SetGlobalFlag(GlobalFlags::fnxml);
                    }
                    else if (arg == "--dtxml" || arg == "-dx")
                    {
                        SetGlobalFlag(GlobalFlags::dtxml);
                    }
