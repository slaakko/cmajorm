...
namespace parser
{
    public class SourceRtf : soul.parsing.Grammar
    {
        public SourceRtf()
            : base("SourceRtf", "Parsers.SourceRtf")
        {
            Link();
        }
        public SourceRtf(ParsingDomain parsingDomain)
            : base("SourceRtf", "Parsers.SourceRtf", parsingDomain)
        {
            Link();
        }
        ...
        {
            AddRule(new SourceRtfRuleParser("SourceRtf", "Parsers.SourceRtf.SourceRtf",
                new SequenceParser(
                    new SequenceParser(
                        new ActionParser("A0",
                            new EmptyParser()),
                    ...
                    new ActionParser("A1",
                        new EmptyParser()))));
            AddRule(new SourceTokenRuleParser("SourceToken", "Parsers.SourceRtf.SourceToken",
                new AlternativeParser(
                    new AlternativeParser(
                        new AlternativeParser(
                            ...
                            new ActionParser("A7",
                                new NonTerminalParser("bct", "BlockComment.bct", "BlockComment", 0))),
                        new ActionParser("A8",
        ...
        private class SourceRtfRuleParser : soul.parsing.RuleParser
        {
            private class Context
            {
                public SourceFormatter formatter;
            }
            public SourceRtfRuleParser(string name, string fullName, Parser definition)
        ...
        private class SourceTokenRuleParser : soul.parsing.RuleParser
        {
            ...
            public override void Link()
            {
                ...
                NonTerminalParser bctNonTerminalParser = GetNonTerminal("bct");
                bctNonTerminalParser.PostCall = Postbct;
            ...
            {
                context.formatter.BlockComment(context.frombct);
            }
            private void A8Action(string match, string content, Position position, string fileName, ref bool pass)
            {
                context.formatter.NewLine();
            }
            private void A9Action(string match, string content, Position position, string fileName, ref bool pass)
            {
            ...
            private void Postbct(Stack<object> stack, bool matched)
            {
                if (matched)
                {
                    context.frombct = (string)stack.Pop();
                }
            }
